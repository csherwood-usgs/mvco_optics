...
for icase = 1:length(cases)
   ...
   if(sum(ok)>3)
      ...
      Cpest_nl3 = pfnl3.Ca*(z(ok)./za).^pfnl3.p; % LISST
      cres_nl =  c(ok)-Cpest_nl;
      dres_nl2 = d(ok)-Cpest_nl2;
      eres_nl3 = e(ok)-Cpest_nl3;
      
      if(1) % kludge: activate to plot residuals
         ecol = [0 0 0]
         subplot(2,3,1)
         plot([0,0],[.1,2],'--k')
         hold on
         plot(eres_nl3,z(ok),'o','color',ecol,'linewidth',2);
         plot(10*dres_nl2,z(ok),'o','color',[1,0 0],'linewidth',2);
         plot(cres_nl,z(ok),'ob','linewidth',2);
         plot(eres_nl3,z(ok),'-','color',ecol,'linewidth',2);
         plot(10*dres_nl2,z(ok),'-','color',[1,0 0],'linewidth',2);
         plot(cres_nl,z(ok),'-b','linewidth',2);
         set(gca,'YLim',[.1 2])
         hx=xlabel('{\color{red}{\itb_{bp}(650)}}, {\color{blue}{\itc_{p}(650)}}, and {\itc}_{{\itp} LISST} [m^{-1}]',...
            'fontsize',14);
         title('Residuals')
         set(hx,'Color','k')
      else % normal plots used in paper
   ...
   if(sum(ok)>3)
      datestr(t)
      subplot(2,3,6)
      if(1) % special plots of LISST sizes during review
         ...
         % plot scale bar
         sb = pz+sf*[-5,5];
         plot([1.75 1.75],sb,'-','linewidth',3,'color',[.5 .5 .5])
         set(gca,'XScale','log')
         xlabel('LISST Size (\mum)')
         set(gca,'fontsize',11)
         title('LISST Volume Concentration')
         %set(gca,'YLim',[.1 2])
         %set(gca,'Yticklabels',[])
         
      else  % normal plot used for first submission
         % plot rho-1 in blue
         hp=plot(c(ok),z(ok),'ob');
         set(hp,'markerfacecolor',[0 0 1],'markeredgecolor',[0,0,1])
         hold on
